 ...
 *
 * Starting books break the page on a line holding exactly `%PAGE%`. Newlines are content, so an
 * author can write blank lines anywhere and in any number — the marker deliberately shares no
 * alphabet with the text (it was `\n\n`, then `\n\n\n`; both capped how many blank lines fit).
 *
...
/** The starting-book page break: a line holding exactly `%PAGE%`, plus one newline on each side. */
export const PAGE_BREAK = /(?:\r?\n)?[ \t]*%PAGE%[ \t]*(?:\r?\n)?/;

/** What the page tools write when they insert a break. */
...
/**
 * StartingBookFactory.paginateExplicit — a `%PAGE%` line is the page break; everything else, blank
 * lines included, is the page verbatim (no per-page strip: the author's newlines are the layout).
 * An empty chunk between two markers is an intentional blank-page slot. Only an oversize chunk falls
 * back to the flow packer. Leading / trailing blank pages are trimmed; internal blanks survive.
 */
function explicitPages(body) {
  const pages = [];
  let cursor = 0;
  for (const chunk of splitSpans(body, new RegExp(PAGE_BREAK.source, 'g'))) {
    const { start, end } = chunk;
    const page = body.slice(start, end);
    if (page.length <= MAX_CHARS_PER_PAGE) {
      pages.push({ text: page, start, end, exclusive: true });
    } else {
      // Same oversize fallback the mod uses (BookFactory.paginate over the chunk).
      for (const p of paginate(page)) pages.push({ text: p, start, end, exclusive: false });
    }
    cursor = end;
  }
  while (pages.length && !pages[0].text.trim()) pages.shift();
  while (pages.length && !pages[pages.length - 1].text.trim()) pages.pop();
  return pages;
}
