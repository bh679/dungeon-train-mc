...
the sibling mods **AIN/AIS/PMOB/ECP/TE/KT/DB/SFF are un-bundled required downloads**, declared `<slug>(required)` so the
...
- **Sibling-mod floors:** AIN/AIS/PMOB/ECP/TE/KT/DB/SFF each have TWO versions in `gradle.properties`.
