...
public final class FabricClientEventBridges {
    ...
    private static void registerTicksAndConnection() {
        ClientTickEvents.START_CLIENT_TICK.register(client -> {
            DtFire.fire(DtEvents.CLIENT_TICK_PRE.listeners(), cb -> cb.onClientTick());
        });
        ClientTickEvents.END_CLIENT_TICK.register(client -> {
            DtFire.fire(DtEvents.CLIENT_TICK_POST.listeners(), cb -> cb.onClientTick());
        });
        ClientPlayConnectionEvents.JOIN.register((handler, sender, client) -> {
            DtFire.fire(DtEvents.CLIENT_LOGGING_IN.listeners(), cb -> cb.onClientLogging());
        });
        ClientPlayConnectionEvents.DISCONNECT.register((handler, client) -> {
            DtFire.fire(DtEvents.CLIENT_LOGGING_OUT.listeners(), cb -> cb.onClientLogging());
        });
        ClientSendMessageEvents.CHAT.register(message -> {
            DtFire.fire(DtEvents.CLIENT_CHAT.listeners(), cb -> cb.onClientChat());
        });
    }
    ...
    private static void registerWorldRenderAndTooltip() {
        WorldRenderEvents.AFTER_TRANSLUCENT.register(context -> {
            ...
            DtFire.fire(DtEvents.RENDER_LEVEL_AFTER_TRANSLUCENT.listeners(),
                cb -> cb.onRenderLevel(poseStack, context.camera(), delta));
        });
        ItemTooltipCallback.EVENT.register((stack, context, type, lines) -> {
            DtFire.fire(DtEvents.ITEM_TOOLTIP.listeners(), cb -> cb.onItemTooltip(stack, lines));
        });
    }
    ...
    private static void registerScreens() {
        ScreenEvents.AFTER_INIT.register((client, screen, scaledWidth, scaledHeight) -> {
            ...
            ScreenEvents.beforeRender(screen).register((scr, graphics, mouseX, mouseY, tickDelta) -> {
                DtFire.fire(DtEvents.SCREEN_RENDER_PRE.listeners(), cb -> cb.onScreenRenderPre(scr));
            });
            ScreenEvents.afterRender(screen).register((scr, graphics, mouseX, mouseY, tickDelta) -> {
                DtFire.fire(DtEvents.SCREEN_RENDER_POST.listeners(), cb -> cb.onScreenRenderPost(scr, graphics));
            });
            ScreenEvents.remove(screen).register(scr -> {
                DtFire.fire(DtEvents.SCREEN_CLOSING.listeners(), cb -> cb.onScreenClosing(scr));
            });
        });
    }
    ...
    private static void fireScreenInit(Screen screen) {
        ...
        DtFire.fire(DtEvents.SCREEN_INIT_POST.listeners(), cb -> cb.onScreenInit(carrier));
    }

    private static void registerInput() {
        // LeftClickBlock — two tiers (HIGHEST menu cancellers, NORMAL observer). Return FAIL to cancel.
        AttackBlockCallback.EVENT.register((player, world, hand, pos, direction) -> {
            if (DtFire.fireCancellable(DtEvents.LEFT_CLICK_BLOCK.listeners(DtPriority.HIGHEST),
                    cb -> cb.onLeftClickBlock(world, pos))) {
                return InteractionResult.FAIL;
            }
            if (DtFire.fireCancellable(DtEvents.LEFT_CLICK_BLOCK.listeners(DtPriority.NORMAL),
                    cb -> cb.onLeftClickBlock(world, pos))) {
                return InteractionResult.FAIL;
            }
    ...
    private static void registerDeclarative() {
        // Key mappings.
        DtFire.fire(DtEvents.KEY_MAPPING_REGISTRATION.listeners(), cb -> cb.registerKeyMappings(KeyBindingHelper::registerKeyBinding));
        ...
        DtFire.fire(DtEvents.GUI_LAYER_REGISTRATION.listeners(), cb -> cb.registerGuiLayers(guiRegistrar));
        ...
        DtFire.fire(DtEvents.CLIENT_TOOLTIP_FACTORY_REGISTRATION.listeners(), cb -> cb.registerFactories(tooltipRegistrar));
