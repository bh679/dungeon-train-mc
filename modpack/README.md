...
| `overrides/` | Copied into the player's instance on install. Shared by both packs. Currently ships `config/smoothswapping.json` (tuned Smooth Swapping), plus the Chinese localization compat pack — see below. |
| `overrides/resourcepacks/DungeonTrain-zh_cn-compat.zip` | zh_cn translations for the bundled **companion** mods (Jade, Tectonic, Distant Horizons, Controlling, ModernFix, CreativeCore, Sable). Dungeon Train's own namespaces (+ AIN/PlayerMob/DiscordPresence) ship their zh_cn `lang/` inside the mod jar, so they're not in here. `overrides/options.txt` enables this pack by default. ⚠ `options.txt` seeds cleanly on a fresh instance but can reset other Minecraft options if the launcher overwrites it on pack update. |
