...
final class WorldGenCycleTest {
    ...
    @Test
    @DisplayName("spheres End sky: 0 through the fade + first half, End across the second half, overworld again at the core's far edge")
    void spheresEndSkyRamp() {
        assertEquals(0.0, C.spheresEndSkyRamp(3390, 50), EPS);            // disabled band → inert

        // Core [4690,5090), len 400 → midpoint world-X 4890.
        WorldGenCycle c = new WorldGenCycle(1000L, 300, 40, new int[] {1, 5, 20}, 0, 60, 50, 200,
                100, 40, 200, 50, 200, 150, 0, 500, 200, 300, 0.12, 0.5, 400, 200, 100, 0);
        assertEquals(0.0, c.spheresEndSkyRamp(4450, 50), EPS);            // lead gap
        assertEquals(0.0, c.spheresEndSkyRamp(4600, 50), EPS);            // entry fade
        assertEquals(0.0, c.spheresEndSkyRamp(4690, 50), EPS);            // core start
        assertEquals(0.0, c.spheresEndSkyRamp(4889, 50), EPS);            // last first-half column

        // Fade in over 50 blocks from the midpoint, hold, fade out over the last 50.
        assertEquals(1.0 / 50, c.spheresEndSkyRamp(4890, 50), EPS);
        assertEquals(1.0, c.spheresEndSkyRamp(4939, 50), EPS);
        assertEquals(1.0, c.spheresEndSkyRamp(5000, 50), EPS);
        assertEquals(1.0, c.spheresEndSkyRamp(5040, 50), EPS);
        assertEquals(1.0 / 50, c.spheresEndSkyRamp(5089, 50), EPS);       // last core column
        assertEquals(0.0, c.spheresEndSkyRamp(5090, 50), EPS);            // back in plain overworld

        // fade 0 = hard switch; oversized fade clamps to a quarter of the core (100).
        assertEquals(0.0, c.spheresEndSkyRamp(4889, 0), EPS);
        assertEquals(1.0, c.spheresEndSkyRamp(4890, 0), EPS);
        assertEquals(1.0, c.spheresEndSkyRamp(5089, 0), EPS);
        assertEquals(1.0 / 100, c.spheresEndSkyRamp(4890, 1000), EPS);
        assertEquals(1.0, c.spheresEndSkyRamp(4989, 1000), EPS);

        // Never touches the End band itself, and repeats with the period.
        assertEquals(0.0, c.spheresEndSkyRamp(2500, 50), EPS);
        int period = (int) c.period();
        assertEquals(1.0, c.spheresEndSkyRamp(5000 + period, 50), EPS);
    }
