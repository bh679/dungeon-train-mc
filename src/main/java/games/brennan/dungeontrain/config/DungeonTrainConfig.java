...
public final class DungeonTrainConfig {
    ...
    private static Holder build(ModConfigSpec.Builder b) {
        ...
        ModConfigSpec.BooleanValue firstLevelEasyMobs = b
                .comment("When true, hostile (Enemy) mobs authored into carriage interiors are replaced with small slimes (or small magma cubes for nether/raider mobs, per the dungeontrain:first_band_magma_mobs entity-type tag; piglin-family mobs in dungeontrain:first_band_piglin_mobs only become magma cubes in the Nether and otherwise spawn as authored) while the run is still in the first tier band (raw tier 0: the first carriagesPerTier carriages), giving an easy combat intro. Passive/neutral carriage mobs (villagers, animals, PlayerMobs) are unaffected. Like firstLevelStarterLoot this uses the raw, un-delayed tier (independent of progressionLevelDelay).")
