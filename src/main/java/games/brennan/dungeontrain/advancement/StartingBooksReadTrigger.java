...
/**
 * Fires when a player's cumulative starting-book read count reaches a
 * threshold. Backing counter lives in {@link GlobalPlayerStats}
 * ({@code startingBooksRead}). Re-reads count — each held right-click
 * on a starting-book item increments by one.
 *
 * <p>JSON shape:
 * <pre>{@code
 * { "trigger": "dungeontrain:starting_books_read",
 *   "conditions": { "thresholdReads": 10 } }
 * }</pre>
 */
public final class StartingBooksReadTrigger extends SimpleCriterionTrigger<StartingBooksReadTrigger.Instance> {
