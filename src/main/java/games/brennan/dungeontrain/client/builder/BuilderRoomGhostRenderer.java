...
public final class BuilderRoomGhostRenderer {
    ...
    private static void rescan() {
        ...
        // The same switch the carriage ghosts read. One control for "draw the builder's ghosts",
        // because from the player's side that is one thing — two settings for two renderers drawing
        // the same idea would be a distinction only the code cares about. A room has no scenery of
        // its own to make solid, so Solid and None come to the same thing here: no ghosts.
