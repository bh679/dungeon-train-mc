...
/**
 * Client-side cache of the endless dimensional carriage the player is in, mirroring {@link ClientNetherBand}.
 *
 ...
 *
 * <p>Pure logic, no rendering imports — {@link DimensionalCarriageFogEvents} is what talks to the renderer.</p>
 */
public final class ClientDimensionalCarriageFog {
    ...
    private static volatile DimensionalCarriageFogPacket region = DimensionalCarriageFogPacket.none();
    ...
    private ClientDimensionalCarriageFog() {}

    /** Apply a server update. */
    public static void update(DimensionalCarriageFogPacket packet) {
        region = packet == null ? DimensionalCarriageFogPacket.none() : packet;
    }

    /** Forget everything. Wired to logging out, so a room never leaks into the next world. */
    public static void reset() {
        region = DimensionalCarriageFogPacket.none();
        applied = 0.0f;
    }
     ...
     * walls, closing toward {@link DimensionalCarriageFogPacket#minRadius} the further out into the swept
     * clearance they get.
     *
    ...
    private static float radiusAt(double x, double z) {
        DimensionalCarriageFogPacket r = region;
