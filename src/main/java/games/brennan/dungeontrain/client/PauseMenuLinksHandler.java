...
/**
 * Replaces the pause menu's vanilla <b>Feedback</b> | <b>Report Bugs</b> row with a single
 * full-width <b>Support the Mod</b> — the title screen's call-to-action, brought to where a
 * player actually spends their time.
 *
 * <p>The support link previously lived only on the title screen ({@link TitleScreenSupportButton}),
 * which a player never sees again once a run is underway. The vanilla row this takes over points at
 ...
 * the natural slot, exactly as Realms is on the title screen. Discord shared this row as a text
 * button until it moved, as an icon, to the Mods | Shaders row's icon strip
 * ({@link games.brennan.dungeontrain.client.menu.PauseMenuIconRow}).</p>
 *
...
public final class PauseMenuLinksHandler {
    ...
    public static void onScreenInitPost(ScreenEvent.Init.Post event) {
        ...
        if (feedback == null || reportBugs == null) {
            LOGGER.warn("PauseMenuLinks: could not locate the Feedback/Report Bugs row (feedback={}, reportBugs={}); skipping Support button.",
                    feedback != null, reportBugs != null);
            return;
        }

        // One button across both slots: from Feedback's left edge to Report Bugs' right edge.
        int x = feedback.getX();
        int y = feedback.getY();
        int w = reportBugs.getX() + reportBugs.getWidth() - x;
        int h = feedback.getHeight();

        event.removeListener(feedback);
        event.removeListener(reportBugs);

        event.addListener(new DarkTintedButton(x, y, w, h, SUPPORT_LABEL, b -> {
        ...
        if (!loggedSwap) {
            loggedSwap = true;
            LOGGER.info("PauseMenuLinks: replaced Feedback/Report Bugs with Support the Mod.");
        }
    }
