...
/**
 * Counters for the three no-sync-load guards (#1450), flushed to the log at most once a minute
 * so a player's {@code latest.log} shows whether the guards are firing without spamming it.
 *
 * <p>Three counters: fluid ticks deferred ({@code FlowingFluidNoSyncLoadMixin}), lava/water
 * interaction checks skipped ({@code FluidInteractionNoSyncLoadMixin}) and physics neighbour
...
public final class NoSyncLoadStats {
    ...
    private static final AtomicLong DEFERRED_FLUID_TICKS = new AtomicLong();
    ...
    public static void fluidTickDeferred() { bump(DEFERRED_FLUID_TICKS); }
    ...
    private static void bump(final AtomicLong counter) {
        ...
        LOGGER.info("[DungeonTrain] [fluid.nosync] last {}s: deferred {} fluid ticks, skipped {} lava/water checks, "
                        + "{} physics reads of unloaded chunks answered as air (server thread never sync-loaded a chunk for them)",
                (now - start) / 1000, DEFERRED_FLUID_TICKS.getAndSet(0), SKIPPED_INTERACTIONS.getAndSet(0),
