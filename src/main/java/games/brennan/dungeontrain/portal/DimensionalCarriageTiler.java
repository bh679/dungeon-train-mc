...
/**
 * Keeps the copies of a dimensional carriage standing around it — the working half of
 * {@link DimensionalCarriageMode#ENDLESS_REPETITION} and {@link DimensionalCarriageMode#ENDLESS_OPEN}.
 *
 ...
 * ({@link DimensionalCarriageTiling#APPROACH_RADIUS}) for a player approaching the carriage, the full window
 ...
 * <ul>
 *   <li><b>The budget is spent</b> — see {@link DimensionalCarriageTiling#budgetTiles}.</li>
 ...
 * face without one is closed</b> — except in {@link DimensionalCarriageMode#ENDLESS_OPEN}, which never closes,
 ...
 */
public final class DimensionalCarriageTiler {
    ...
    /** The four ways out of a room. Y is not among them — see {@link DimensionalCarriageMode}. */
    private static final int[][] DIRECTIONS = {{1, 0}, {-1, 0}, {0, 1}, {0, -1}};

    private DimensionalCarriageTiler() {}
     ...
     * @param radius     how far the window reaches — {@link DimensionalCarriageTiling#MAX_RADIUS} for somebody
     *                   actually in the room, {@link DimensionalCarriageTiling#APPROACH_RADIUS} for somebody
    ...
    public static PortalStructure tick(ServerLevel level, CarriageDims dims,
        ...
        DimensionalCarriageTiling tiling = structure.tiling();
     ...
     *
     * <p>{@link DimensionalCarriageMode#ENDLESS_OPEN} repeats the floor and the roof, in the authored room's
    ...
    private static PortalStructure stampTile(ServerLevel level, CarriageDims dims,
        ...
        PortalCarriageBuilder.stampRoomAt(level, origin, dims, structure.roomName(), size,
            ...
            pairKey, tile,
            DimensionalCarriageMobs.liveCount(level, PortalCarriageBuilder.footprintOf(level, structure, dims), pairKey),
     ...
     * {@link DimensionalCarriageMode#ENDLESS_OPEN} — everything strictly between the tile's floor and its
     * ceiling.
     *
     * <p>Tested against {@code ENDLESS_OPEN} rather than {@code !tilesWholeRoom()}, which is also
     * true of {@link DimensionalCarriageMode#BEDROCK_LOCK} and is only unreachable for it because
    ...
    static PortalCorridorMask writeMaskFor(PortalStructure structure,
        ...
        if (structure.mode() != DimensionalCarriageMode.ENDLESS_OPEN) return clearMask;
     ...
     *
     * <p>The row test covers the pair alone. An extra corridor ({@link DimensionalCarriageExits}) can stand on
    ...
    private static PortalCorridorMask maskFor(PortalStructure structure, CarriageDims dims,
        ...
        // stood on row zero; a pair that moved its exit (DimensionalCarriageExits) can have it beside any tile
    ...
    private static PortalStructure eraseTile(ServerLevel level, CarriageDims dims,
        ...
        DimensionalCarriageMobs.reapTile(level, box, pairKey, tile);
    ...
    /** Take a face away — {@link DimensionalCarriageMode#ENDLESS_OPEN} only. */
    ...
    private static boolean canStamp(ServerLevel level, CarriageDims dims, PortalStructure structure,
        ...
        for (PortalStructure other : neighbours) {
            ...
            if (box.intersects(PortalCarriageBuilder.footprintOf(level, other, dims))) {
                LOGGER.debug("[DungeonTrain] Dimensional carriage copy at {} skipped — it would land on another pair",
                    tile);
                return false;
            }
        }
