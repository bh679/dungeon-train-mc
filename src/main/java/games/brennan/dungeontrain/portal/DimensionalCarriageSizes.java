...
/**
 * How big each named dimensional carriage is, without needing a {@code ServerLevel} to ask.
 *
 ...
 */
public final class DimensionalCarriageSizes {
    ...
    private DimensionalCarriageSizes() {}
    ...
    public static Vec3i sizeOf(String name, CarriageDims dims) {
        if (name == null) return DimensionalCarriageLayout.builtInSize(dims);
        Vec3i pending = PENDING.get(name);
        if (pending != null) return DimensionalCarriageLayout.clampSize(dims, pending);
        Vec3i known = SIZES.get(name);
        return known != null
            ? DimensionalCarriageLayout.clampSize(dims, known)
            : DimensionalCarriageLayout.builtInSize(dims);
    }
