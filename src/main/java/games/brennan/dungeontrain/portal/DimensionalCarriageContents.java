...
/**
 * Whether a dimensional carriage is furnished from the ordinary contents pool, and how a furnishing that does
 ...
 *
 * <p>Stored as the third segment of the room's {@code mode} tag — see {@link DimensionalCarriageSettings},
 * which owns the encoding.</p>
 */
public enum DimensionalCarriageContents {
    ...
    public static final DimensionalCarriageContents DEFAULT = OFF;

    private final String id;
    private final String displayName;

    DimensionalCarriageContents(String id, String displayName) {
        this.id = id;
     ...
     *
     * <p>Total, for the same reason {@link DimensionalCarriageMode#parse} and {@link DimensionalCarriageCopies#parse}
     ...
     */
    public static DimensionalCarriageContents parse(String id) {
        if (id == null) return DEFAULT;
        String key = id.trim().toLowerCase(Locale.ROOT);
        if (key.isEmpty()) return DEFAULT;
        for (DimensionalCarriageContents c : values()) {
    ...
    public DimensionalCarriageContents next() {
        DimensionalCarriageContents[] all = values();
