...
/**
 * Whether the copies {@link DimensionalCarriageMode#ENDLESS_REPETITION} makes are identical to the room they
 ...
 */
public enum DimensionalCarriageCopies {
    ...
    public static final DimensionalCarriageCopies DEFAULT = EXACT;

    private final String id;
    private final String displayName;

    DimensionalCarriageCopies(String id, String displayName) {
        this.id = id;
     ...
     *
     * <p>Total, for the same reason {@link DimensionalCarriageMode#parse} is: the tag is free text on disk and
     * a misspelling should stamp a room, not fail a pair.</p>
     */
    public static DimensionalCarriageCopies parse(String id) {
        if (id == null) return DEFAULT;
        String key = id.trim().toLowerCase(Locale.ROOT);
        if (key.isEmpty()) return DEFAULT;
        for (DimensionalCarriageCopies c : values()) {
    ...
    public DimensionalCarriageCopies next() {
        DimensionalCarriageCopies[] all = values();
