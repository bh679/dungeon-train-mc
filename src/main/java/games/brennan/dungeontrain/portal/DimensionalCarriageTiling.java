...
/**
 * Which copies of a dimensional carriage are currently standing, for the two endless
 * {@link DimensionalCarriageMode modes} — a grid of tiles around the base room, in tile coordinates where
 ...
 */
public record DimensionalCarriageTiling(Set<Tile> tiles) {
    ...
    public static final int MAX_RESIDENT_BLOCKS =
        MAX_RADIUS * DimensionalCarriageLayout.MAX_LENGTH * DimensionalCarriageLayout.MAX_HEIGHT * DimensionalCarriageLayout.MAX_WIDTH;
    ...
    public DimensionalCarriageTiling {
    ...
    public static DimensionalCarriageTiling base() {
        return new DimensionalCarriageTiling(Set.of(Tile.BASE));
    }
     ...
     *
     * <p>Sized off the room's <b>whole box</b> even for {@link DimensionalCarriageMode#ENDLESS_OPEN}, which
    ...
    public DimensionalCarriageTiling with(Tile tile) {
        if (tiles.contains(tile)) return this;
        Set<Tile> next = new LinkedHashSet<>(tiles);
        next.add(tile);
        return new DimensionalCarriageTiling(next);
    }

    /** This tiling minus {@code tile}. The base tile is never removed. */
    public DimensionalCarriageTiling without(Tile tile) {
