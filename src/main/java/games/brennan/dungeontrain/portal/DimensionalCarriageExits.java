...
/**
 * How many extra ways back to the train an endless dimensional carriage scatters through its copies, and how
 * far apart they are.
 *
 ...
 * rooms back — and the sliding window ({@link DimensionalCarriageTiling}) is what makes that distance real
 ...
 * <p>{@link DimensionalCarriageMode#ENDLESS_REPETITION} defaults to {@link Kind#ON}: it is a hall of identical
 * rooms, which is exactly the shape a player gets lost in, so extra doors are the kind thing. {@link
 * DimensionalCarriageMode#ENDLESS_OPEN} defaults to {@link Kind#OFF}: it is an open plain with sightlines
 * across it, so the way back stays visible and a corridor standing in the middle of it is furniture
 * nobody asked for. See {@link DimensionalCarriageMode#defaultExits}.</p>
 *
 * <p>Stored as the fourth segment of the room's {@code mode} tag — {@link DimensionalCarriageSettings} owns
 ...
 */
public record DimensionalCarriageExits(Kind kind, int every, int moveChance) {
    ...
    public enum Kind {
         ...
         *
         * <p>Total, for the same reason {@link DimensionalCarriageMode#parse} is: the tag is free text on disk,
         ...
         * DimensionalCarriageSettings#parse} is where an <i>absent</i> segment becomes {@link
         * DimensionalCarriageMode#defaultExits}; a present-but-unreadable one is a different case and lands
         * here.</p>
         */
     ...
     * {@link DimensionalCarriageTiling#MAX_RADIUS} rooms in each direction, so a lattice spaced beyond a few
    ...
    public static final DimensionalCarriageExits OFF = new DimensionalCarriageExits(Kind.OFF, DEFAULT_EVERY, MOVE_NEVER);

    /** The lattice at its default spacing — what an Endless Repetition room gets when it says nothing. */
    public static final DimensionalCarriageExits ON = new DimensionalCarriageExits(Kind.ON, DEFAULT_EVERY, MOVE_NEVER);

    public DimensionalCarriageExits {
    ...
    public DimensionalCarriageExits(Kind kind, int every) {
     ...
     */
    public static DimensionalCarriageExits parse(String segment) {
        ...
        return new DimensionalCarriageExits(
            Kind.parse(parts[0]),
    ...
    public DimensionalCarriageExits next() {
        return new DimensionalCarriageExits(kind.next(), every, moveChance);
    }

    public DimensionalCarriageExits withKind(Kind newKind) {
        return new DimensionalCarriageExits(newKind, every, moveChance);
    }

    public DimensionalCarriageExits withEvery(int newEvery) {
        return new DimensionalCarriageExits(kind, newEvery, moveChance);
    }

    public DimensionalCarriageExits withMoveChance(int newChance) {
        return new DimensionalCarriageExits(kind, every, newChance);
    }
}
