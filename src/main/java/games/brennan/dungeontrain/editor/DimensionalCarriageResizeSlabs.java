...
/**
 * The row a dimensional-carriage resize adds or takes away, and the two sidecars that have to move with it.
 *
 ...
 * into a per-world {@link DimensionalCarriageResizeMemory}; {@code /dt save} remains the only thing that
 * writes a room's files.</p>
 */
final class DimensionalCarriageResizeSlabs {

    private static final Logger LOGGER = LogUtils.getLogger();

    private DimensionalCarriageResizeSlabs() {}

    private static String plotKey(String name) {
        return ContainerContentsStore.trackPlotKey(TrackKind.DIMENSIONAL_CARRIAGE, name);
    }
    ...
    static void stash(ServerLevel overworld, String name, BlockPos origin, Vec3i sizeBefore,
        ...
        DimensionalCarriageResize.Axis axis = step.axis();
        Vec3i slabSize = DimensionalCarriageResize.with(sizeBefore, axis, 1);
        BlockPos slabOrigin = origin.offset(DimensionalCarriageResize.along(axis, step.slabIndex()));
        ...
        TrackVariantBlocks sidecar = TrackVariantBlocks.loadFor(TrackKind.DIMENSIONAL_CARRIAGE, name, sizeBefore);
        TrackVariantBlocks slabSidecar = TrackVariantBlocks.emptyFor(TrackKind.DIMENSIONAL_CARRIAGE);
        int cells = 0;
        for (Map.Entry<BlockPos, Integer> e : slabCells(sidecar, axis, step.slabIndex()).entrySet()) {
            BlockPos local = e.getKey().subtract(DimensionalCarriageResize.along(axis, step.slabIndex()));
        ...
        for (BlockPos pos : List.copyOf(store.allPositions())) {
            if (DimensionalCarriageResize.of(pos, axis) != step.slabIndex()) continue;
            BlockPos local = pos.subtract(DimensionalCarriageResize.along(axis, step.slabIndex()));
        ...
        DimensionalCarriageResizeMemory.get(overworld).put(name, axis, step.memoryKey(),
            new DimensionalCarriageResizeMemory.Slab(blocks.save(new CompoundTag()),
                slabSidecar.isEmpty() ? "" : slabSidecar.asJsonText(),
                pools == 0 ? "" : slabStore.asJsonText()));

        LOGGER.debug("[DungeonTrain] Dimensional carriage '{}': filed the {} row cropped at {}={} "
    ...
    static boolean restore(ServerLevel overworld, String name, BlockPos origin, Vec3i sizeAfter,
                           DimensionalCarriageResize.Step step) {
        DimensionalCarriageResizeMemory.Slab slab =
            DimensionalCarriageResizeMemory.get(overworld).take(name, step.axis(), step.memoryKey());
        if (slab == null) return false;

        DimensionalCarriageResize.Axis axis = step.axis();
        Vec3i offset = DimensionalCarriageResize.along(axis, step.slabIndex());
        ...
        TrackVariantBlocks sidecar = TrackVariantBlocks.loadFor(TrackKind.DIMENSIONAL_CARRIAGE, name, sizeAfter);
        TrackVariantBlocks slabSidecar = TrackVariantBlocks.fromJsonText(
            slab.variantsJson(), TrackKind.DIMENSIONAL_CARRIAGE, name,
            DimensionalCarriageResize.with(sizeAfter, axis, 1));
        ...
        LOGGER.debug("[DungeonTrain] Dimensional carriage '{}': restored the {} row filed at {}={}",
    ...
    private static Map<BlockPos, Integer> slabCells(TrackVariantBlocks sidecar,
                                                    DimensionalCarriageResize.Axis axis, int slabIndex) {
        Map<BlockPos, Integer> out = new LinkedHashMap<>();
        for (CarriageVariantBlocks.Entry e : sidecar.entries()) {
            if (DimensionalCarriageResize.of(e.localPos(), axis) == slabIndex) {
